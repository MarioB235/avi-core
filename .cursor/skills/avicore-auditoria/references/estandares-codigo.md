...
- Snackbar global (`x-ui.snackbar-host`): `tests/Feature/Ui/SnackbarHostTest.php` (layout operario, evento `snackbar-show`, flash `status`, auto-cierre 4500, contrato CSS desktop `right-6`/`bottom-6` en `lg+`).
- PWA (`x-ui.pwa-meta`, `x-ui.pwa-install-prompt`): `tests/Feature/Ui/PwaInstallPromptTest.php` (manifest + banner en login/operario/admin; split `enabled` vs `install_prompt`; `apple-touch-icon` + `pwa-192`).
- Menú cuenta compartido (`x-ui.user-menu`, alias `x-operario.user-menu`): `tests/Feature/Ui/OperarioUserMenuTest.php` (portal/teleport, `syncPanelPosition`, clamp `panelHeight`/`max-height`, Perfil + logout en rutas operario); `tests/Feature/Ui/AdminUserMenuTest.php` (mismo componente en Inicio admin y `/admin/usuarios`: sidebar, home-nav, portal).
