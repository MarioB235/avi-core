...
- Servicios operarios con reglas de negocio (`OperarioGalponService`: `galponDisponibleParaUsuario`, `historialCargasQuery`, multiempresa, galpón disponible; `OperarioGalponResumenService`: `resumen`, ventana de acumulado, maples): tests Feature en `tests/Feature/Services/OperarioGalponServiceTest.php`, `tests/Feature/Operario/OperarioHomeResumenTest.php`, `tests/Feature/Operario/OperarioHistorialTest.php` (tipos, filtro fecha validado, paginación, multiempresa) + integración en `tests/Feature/Ui/OperarioBottomNavTest.php` (heroes, nav activa, historial empty/populated HTTP, icono `calendar`) y `tests/Feature/Support/OperarioNavTest.php` (pestaña activa y `headerTitle` por ruta) + `tests/Feature/Operario/OperarioHomeTest.php` (`seleccionarGalpon` rechaza galpón ajeno, en mantenimiento o inactivo).
