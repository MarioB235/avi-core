...
- Snackbar global (`x-ui.snackbar-host`): `tests/Feature/Ui/SnackbarHostTest.php` (layout operario, evento `snackbar-show`, flash `status`, auto-cierre 4500, contrato CSS desktop `right-6`/`bottom-6` en `lg+`, contrato `pwa.js` → snackbar `pwa-update`).
