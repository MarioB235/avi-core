...
| `x-ui.textarea` | Mismo contrato visual que `x-ui.input` (label, `hint`, `error` o bag `$errors`, `aria-invalid`); usado en motivo de anulación del Historial operario |
...
| `x-ui.snackbar-host` | Toast fijo — `context` (`operario` \| `default`). **Operario éxito:** pill compacto (`avicore-snackbar--compact`) verde suave, ~2,5s, sin barra ni ×; **operario con acción o error:** tarjeta completa con franja, barra de progreso y cierre. **Default/admin:** tarjeta con franja lateral, icono chip, barra superior (~3,5s), × y acción opcional (`pwa-update`). Duración vía `syncProgressDuration()` + `setProperty('--snackbar-duration')` (**sin** `:style` en Blade). Móvil operario: centrado sobre dock; escritorio (`lg+`) abajo-derecha; evento `snackbar-show`; flash `status` + `status_variant` |
...
| `x-ui.reveal` | Bloque de sección con entrada al scroll — prop `as` (default `div`), `delay` opcional (ms) vía `data-reveal-delay` + CSS `attr()` en `app.css` (**sin** `style=` en Blade); `data-avicore-reveal` + `scroll-reveal.js`; no usar en filas de lista/tabla |
