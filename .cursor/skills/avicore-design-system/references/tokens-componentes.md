...
| `x-ui.snackbar-host` | Toast fijo — tarjeta `avicore-card` con franja lateral por variante, icono en chip soft; barra de progreso superior (~2px) con tiempo restante de auto-cierre (pausa al hover/foco; oculta con `prefers-reduced-motion` o si lleva acción); duración dinámica vía `syncProgressDuration()` + `setProperty('--snackbar-duration')` en Alpine (**sin** `:style` en Blade); auto-cierre ~3,5s; barra se vacía hacia la derecha; botón opcional (`actionLabel` + `actionKey`, p. ej. `pwa-update`); cierre manual × o Escape; `context` (`operario` \| `default`); móvil centrado sobre dock / `bottom-6`; escritorio (`lg+`) abajo-derecha; evento `snackbar-show`; flash `status` + `status_variant` |
