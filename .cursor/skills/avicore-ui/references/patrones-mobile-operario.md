...
- `.avicore-operario-home-sheet` — **todos** los registros activos del operario (`registros_operativos` + `vacunaciones`), orden **cronológico descendente** (más reciente arriba). Cabecera y empty con `operario-reloj` en `avicore-operario-carga-tile__icon` (igual que KPIs Inicio y tiles Cargar). Título: «Todos los registros» / «Registros del día» (sin repetir la fecha concreta; la fecha vive solo en el date picker). Lista simple `.avicore-operario-historial-list`: una línea con resumen (`cantidadResumen()` o `Vacunacion::cantidadResumen()`), observación opcional debajo, fecha/hora a la derecha; ítems de mortalidad en `text-avicore-danger` (`.avicore-operario-historial-list__item--muertes`); vacunaciones en verde primario (`.avicore-operario-historial-list__item--vacunacion`). Filtro en bloque suave `.avicore-operario-historial-filter` (`avicore-soft`): `x-ui.date-picker` (placeholder «Elegí un día»); «Ver todas» al lado cuando hay filtro; `?fecha=` validado Livewire (`date`, `before_or_equal:today`); al fallar, `resetErrorBag('fecha')` + un solo mensaje vigente pasado como `fechaError` → `:error` del date-picker (`role="alert"`, trigger `--error`); paginación Livewire (20). Cuenta/logout solo en menú avatar del header.
- Tests: `OperarioHistorialTest` (tipos, vacunaciones mezcladas, filtro, fechas inválidas/futuras con mensaje visible y sin acumulación, paginación, multiempresa, date-picker/`entangle`); `DatePickerComponentTest` (panel, bounds, prop `error`, bag `$errors`, `dayAriaLabel`); `OperarioCargaVacunacionTest`; `OperarioCargaLoteTest` (alta lote, gating operario, administrativo, bordes Livewire); `OperarioGalponServiceTest` (`historialPaginado` con vacunaciones); `OperarioBottomNavTest` (HTTP historial + date-picker + deep link `?form=vacunacion|lote` + ilustración `operario-reloj`); `IllustrationComponentTest`; `OperarioNavTest`; `OperarioUserMenuTest`; `SelectComponentTest`.
